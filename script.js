...
const lengthOfShama = shama.length;

const columnA = shama.map(item => item[0]);
const columnB = shama.map(item => item[1]);
...
shuffleArray(columnB);


// const columnA = ['Red', 'Blue', 'Green', 'Yellow', 'Purple', 'Yo', 'Yomolopo'];
// const columnB = ['Apple', 'Banana', 'Cherry', 'Date', 'Elderberry', 'Fig', 'Grape']
let marks=0; 
console.log("Begining"+marks)


var linkList = [{dragId: "1-draggable", dropId: "1-dropzone", color: "#264653"}];


var linkCorrection = [{dragId: "1-draggable", dropId: "1-dropzone", color: "#33991a"},
                      {dragId: "2-draggable", dropId: "2-dropzone", color: "#4c061d"},
                      {dragId: "3-draggable", dropId: "3-dropzone", color: "#d17a22"},
                      {dragId: "4-draggable", dropId: "4-dropzone", color: "#3b3923"},
                      {dragId: "5-draggable", dropId: "5-dropzone", color: "#3b5249"},
                      {dragId: "6-draggable", dropId: "6-dropzone", color: "#3b5249"},
                      {dragId: "7-draggable", dropId: "7-dropzone", color: "#3b5249"}]
...
/**New  */
function Correction() {
  //  marks = 0; // Initialize marks outside the loop to ensure it accumulates correctly
  linkList.forEach(userR => {
    const test = linkCorrection.find(cor => cor.dragId == userR.dragId && cor.dropId == userR.dropId);
    if (test) {
      userR.color = "green";
  ...
  drawLinks();
  document.getElementById('marksDisplay').innerText = "Your Obtained marks is : " + marks; // Update the marks display
  console.log("Inside are " + marks);

}
console.log("Outside are " + marks);
...
// Function to render the array to HTML
function renderColumnA() {
    // Get the ul element by its ID
    const ul = document.getElementById('dragUL');

    // Iterate over the colors array and create li elements
    columnA.forEach((element, index) => {
        const dragId = `${index + 1}-draggable`;
        const li = `<li>${element}<div id="${dragId}" class="dragElement" draggable="true"><i class="far fa-circle"></i></div></li>`;
        ul.innerHTML += li;
    });
}

// Execute the renderColors function when the DOM is fully loaded
document.addEventListener('DOMContentLoaded', renderColumnA);



// Function to render the array to HTML
function renderColumnB() {
    // Get the ul element by its ID
    const ul = document.getElementById('dropUL');

    columnB.forEach((colB, index) => {
        const dropId = `${index + 1}-dropzone`;

        
        const li = `<li><div id="${dropId}"  onclick="clearPath(event)" class="dragElement dropElement"> <i class="far fa-circle"></i> </div>${colB}</li>
`;
        ul.innerHTML += li;
    });
}
...
// Execute the renderColors function when the DOM is fully loaded
document.addEventListener('DOMContentLoaded', renderColumnB);
